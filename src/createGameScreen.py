...
class CreateGameScreen(Screen):
    ...
    def __init__(self, screenWidth, screenHeight, oldPlayerList=None, oldSocketList=None):
        ...
        if oldSocketList != [] and oldSocketList != None:  # If this is a screen after a previous round
    ...
    # A function which is called when the main menu button is pressed
    def mainMenuSwitch(self):
        self.shouldThread = False  # Stop the thread from threading
        if self.serverSock is not None:  # If we have an open socket
            self.serverSock.close()  # Close it
    ...
    def waitForConnections(self):
        while self.shouldThread:
            ...
            name = clientSocket.recv(1024)  # Receive their name
            self.addNewPlayer(name.decode().replace("_", " "), address[0])  # Add the player to the player list
            clientSocket.sendall(encodePlayers(self.players).encode())  # Send the player list to the player
