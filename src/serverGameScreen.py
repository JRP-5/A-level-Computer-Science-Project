...
class ServerGameScreen(gameScreen.GameScreen):
    ...
    def tickScreen(self):
        ...
        for ent in self.entityList:
            if ent.type == "tank":  # For each tank
                if ent.team not in teams:  # If their team is not in the team list
                    teams.append(ent.team)  # Add it
        if len(teams) == 1:
            self.winnerTicks += 1  # Increment the winner ticks
            for i in range(len(self.entityList)):
                if self.entityList[i].type == "tank":
                    if self.entityList[i].uuid == self.thisTank.uuid:  # If the server's tank has won
                        # Display won text
                    ...
                    elif self.entityList[i].player.Ai is False and self.isLAN:
...
def encodeEntityList(entityList):
    ...
    result += "  " + time  # only get hours mins, secs and milliseconds
